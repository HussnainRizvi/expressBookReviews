...
public_users.get('/author/:author', async (req, res) => {
    try {
        ...
        return res.status(500).json({message: "Failed to fetch books by author", error});
    }
...
public_users.get('/title/:title', async (req, res) => {
    try {
        ...
        return res.status(500).json({message: "Failed to fetch books by title", error});
    }
});
...
public_users.get('/review/:isbn',function (req, res) {
  ...
  if (book && book.reviews) {
    ...
    return res.status(400).json({ message: "No reviews found by this book"})
  }
});
